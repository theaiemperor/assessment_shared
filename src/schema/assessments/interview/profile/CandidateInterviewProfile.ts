...
export const CandidateInterviewProfileCoreZ = z.object({
    ...
    resume: z.string().url("Please Provide a valid url")
        .max(2048, "URL cannot be so long"),

    resumeContent: z.string(),

    experience: z.number()
        .min(0, "Experience cannot be negative.")
        .max(50, "It is too much experience to have.")
    ...
    skills: z.array(z.string())
        .min(1, "Please provide minimum one skill")
        .max(100, "Please use short name for the skill")
        .describe("Array of user's skills"),

    education: z.array(z.object({

        qualification: z.string().nonempty("Please provide qualification, degree or diploma name.")
        ...
        source: z.string().nonempty("Please provide source from where you completed this degree")
            .describe('It can be school, university, online course, any skill programme anything from there qualification was taken.'),

        year: z.number().gt(1950, "Valid Year of completion required. it can be in future year if still pending.")
    ...
    history: z.array(z.object({
        role: z.string()
            .min(3, "Please provide a long name")
            .max(500, "Please provide a short name")
        ...
        duration: z.number()
            .min(1, "Minimum one month work is required to add in previous experience")
            .max(350, "This is too much time to spend in a single workplace")
